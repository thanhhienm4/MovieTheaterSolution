...
namespace MovieTheater.Models.User
{
    public class UserRegisterRequest
    {
        ...
        [Display(Name = "Mật khẩu")] public String Password { get; set; }

        [Display(Name = "Xác nhận mật khẩu")] public String ConfirmPassword { get; set; }
    ...
    public class UserCreateValidator : AbstractValidator<UserRegisterRequest>
    {
        ...
        {
            RuleFor(x => x.UserName).NotEmpty().WithMessage("Tên đăng nhập không được bỏ trống")
                .Must(u => !u.Any(x => Char.IsWhiteSpace(x))).When(u => !string.IsNullOrWhiteSpace(u.UserName))
                .WithMessage("Tên đăng nhập không bao gồm khoảng trắng");
            ...
            RuleFor(x => x.Email).NotEmpty().WithMessage("Email không được bỏ trống ").EmailAddress()
            ...
            RuleFor(x => x.PhoneNumber).NotEmpty().WithMessage("Số điện thoại không được để trống")
                .Matches("^0[0-9]{9}$").WithMessage("Số điện thoại 10 số bắt đầu từ số 0");
            RuleFor(x => x.Password).NotEmpty().WithMessage("Mật khẩu không được để trống")
                .Matches("^(?!^[0-9]*$)(?!^[a-zA-Z]*$)^([a-zA-Z0-9]{8,15})$")
